        ...
        public string GetPropertiesObject()
        {
            ...
            if (propertyArray != null && propertyArray.Length > 0)
            {
                foreach (PropertyInfo property in propertyArray)
                {
                    ...
                    if (pvalue != null && pvalue.GetType().Namespace == "GenmCloud.Shared.Common.Query")
                    {
                    ...
                    else if (pvalue != null && (pvalue.GetType().Namespace.Contains("GenmCloud.Shared.Dto") || pvalue.GetType().Namespace.Contains("GenmCloud.Shared.HttpContact")))
                    {
                        //当属性为对象得情况下, 进行序列化
